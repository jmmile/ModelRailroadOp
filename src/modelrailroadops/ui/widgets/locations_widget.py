...
class LocationsWidget(QWidget):
    """Manage railroad locations and their operational tracks."""
    ...
    def __init__(self, parent=None):
        ...
        self.location_model.setHorizontalHeaderLabels(
            ["Name", "Type", "Tracks", "Status", "Notes"]
        )

        self.location_table = QTableView()
        self._configure_table(self.location_table, self.location_model)
        ...
        self.edit_track_button = QPushButton("Edit Track")
        ...
        self._configure_table(self.track_table, self.track_model)
        track_layout.addWidget(self.track_table)

        splitter = QSplitter(Qt.Vertical)
        splitter.setChildrenCollapsible(False)
        splitter.addWidget(self.location_table)
        splitter.addWidget(track_area)
        ...
        self.activate_location_button.clicked.connect(
            lambda: self.set_location_active(True)
        )
        self.deactivate_location_button.clicked.connect(
            lambda: self.set_location_active(False)
        )
        ...
        self.location_table.doubleClicked.connect(self.edit_location)
        self.track_table.doubleClicked.connect(self.edit_track)
    ...
    def _configure_table(table, model):
        ...
        table.setStyleSheet(TABLE_SELECTION_STYLE)
    ...
    @staticmethod
    def _row_items(values, record_id):
        items = []
        for value in values:
            item = QStandardItem()
            item.setData(
    ...
    def refresh(self):
        selected_id = self.get_selected_location_id()
        self.locations = list(LocationService.get_all())
        self.location_model.removeRows(0, self.location_model.rowCount())
        ...
        self.location_status.setText(f"{len(self.locations)} locations")

        row_to_select = 0 if self.locations else None
        if selected_id is not None:
            for row in range(self.location_model.rowCount()):
                item = self.location_model.item(row, 0)
                if (
                    item is not None
                    and item.data(Qt.UserRole) == selected_id
                ):
                    row_to_select = row
                    break

        if row_to_select is not None:
            self.location_table.selectRow(row_to_select)
        else:
            self.load_tracks()
    ...
    def get_selected_location(self):
        rows = self.location_table.selectionModel().selectedRows()
        if not rows:
            return None
        row = rows[0].row()
        item = self.location_model.item(row, 0)
        location_id = item.data(Qt.UserRole) if item is not None else None
    ...
    def location_selection_changed(self, selected=None, deselected=None):
        self.load_tracks()
        self.update_buttons()

    def load_tracks(self):
        location = self.get_selected_location()
        self.tracks = list(location.tracks) if location is not None else []
        self.track_model.removeRows(0, self.track_model.rowCount())

        if location is None:
            self.track_label.setText("Tracks")
        else:
            self.track_label.setText(f"Tracks — {location.name}")
            ...
            self.track_model.appendRow(
                self._row_items(
                    [
                        ...
                        track.capacity if track.capacity is not None else "",
                        "Active" if track.active else "Inactive",
                        track.notes or "",
                    ],
                    track.id,
                )
    ...
    def update_buttons(self, selected=None, deselected=None):
        location = self.get_selected_location()
        track = self.get_selected_track()
        has_location = location is not None
        industry_location = bool(location.industries) if location else False
        linked_industry_track = bool(track.industry_tracks) if track else False

        self.edit_location_button.setEnabled(has_location)
        self.delete_location_button.setEnabled(
            has_location and not industry_location
        )
        self.activate_location_button.setEnabled(
            has_location and not location.active
        )
        self.deactivate_location_button.setEnabled(
            has_location and location.active
        )
        self.add_track_button.setEnabled(has_location and not industry_location)
        self.edit_track_button.setEnabled(track is not None and not linked_industry_track)
        self.delete_track_button.setEnabled(track is not None and not linked_industry_track)

    def add_location(self):
        if AddLocationDialog(self).exec() == QDialog.DialogCode.Accepted:
            self.refresh()

    def edit_location(self, index=None):
        location = self.get_selected_location()
        if location is None:
            return
        if AddLocationDialog(self, location).exec() == QDialog.DialogCode.Accepted:
            self.refresh()

    def set_location_active(self, active):
        location = self.get_selected_location()
        if location is None:
            return
        success, result = LocationService.set_active(location.id, active)
        if not success:
    ...
    def delete_location(self):
        ...
        answer = QMessageBox.question(
            ...
            (
                ...
                f"Its {len(location.tracks)} general track(s) will also be "
        ...
        success, result = LocationService.delete(location.id)

        if not success:
            QMessageBox.warning(self, "Delete Location", str(result))
            return
    ...
    def add_track(self):
        ...
        if location is None:
            return
        dialog = AddLocationTrackDialog(location, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()

    def edit_track(self, index=None):
        location = self.get_selected_location()
        track = self.get_selected_track()
        if location is None or track is None:
            return
        dialog = AddLocationTrackDialog(location, self, track)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()
    ...
    def delete_track(self):
        ...
        if answer != QMessageBox.Yes:
            return
        success, result = LocationService.delete_track(track.id)
        if not success:
