...
class OperationsSessionService:
    ...
    def get_by_id(session_id):
        if session_id is None:
            return None
        with SessionLocal() as session:
            return session.get(OperationsSession, session_id)

    @staticmethod
    def create(name, session_date, notes=None):
        ...
        with SessionLocal() as session:
            ...
            session.commit()
            session.refresh(operations_session)
            return True, operations_session

    @staticmethod
    def update(session_id, name, session_date, notes=None):
        if session_id is None:
            return False, "No Operations Session was specified."
        name = name.strip() if name else ""
        if not name:
            return False, "Operations Session name is required."
        if session_date is None:
        ...
        with SessionLocal() as session:
            ...
            if operations_session is None:
                return False, f"Operations Session {session_id} was not found."
    ...
    def start(session_id):
        ...
        with SessionLocal() as session:
            ...
            if operations_session.status != "PLANNED":
                return False, "Only a PLANNED Operations Session can be started."
    ...
    def complete(session_id):
        """Complete a session only when every unfinished Waybill arrives.

        The preflight validates all Waybills before any status changes.  The
        subsequent Waybill and session changes share one transaction, so a
        failed completion never leaves a partially completed session.
        """
        if session_id is None:
            return False, "No Operations Session was specified."
        with SessionLocal() as session:
            ...
            if operations_session.status != "ACTIVE":
                return False, "Only an ACTIVE Operations Session can be completed."

            unfinished_waybills = session.execute(
                select(Waybill)
                .where(
            ...
            for waybill in unfinished_waybills:
                ...
                if not valid:
                    incomplete.append(f"Waybill #{waybill.id}: {message}")

            if incomplete:
                return False, (
                    "The Operations Session cannot be completed because "
                    "these Waybills are unfinished:\n\n"
                    + "\n".join(incomplete)
                )

            for waybill in unfinished_waybills:
                success, result = WaybillService.complete(
                    waybill.id,
                    db_session=session,
                )
                if not success:
            ...
            operations_session.completed_at = datetime.utcnow()
            session.commit()
            session.refresh(operations_session)
            return True, operations_session
    ...
    def cancel(session_id):
        ...
        with SessionLocal() as session:
            ...
            if operations_session.status not in ("PLANNED", "ACTIVE"):
                return False, (
                    "Only a PLANNED or ACTIVE Operations Session can be "
                    "cancelled."
                )
    ...
    def delete(session_id):
        ...
        with SessionLocal() as session:
            ...
            if operations_session is None:
                return False, f"Operations Session {session_id} was not found."
            session.delete(operations_session)
            session.commit()
            return True, "Operations Session deleted successfully."

    @staticmethod
    def get_waybills_by_session(session_id):
        if session_id is None:
            return []
        with SessionLocal() as session:
            return session.execute(
                ...
                .where(Waybill.operations_session_id == session_id)
                .order_by(Waybill.id)
            ).scalars().unique().all()

    @staticmethod
    def get_waybills(session_id):
        return OperationsSessionService.get_waybills_by_session(session_id)
